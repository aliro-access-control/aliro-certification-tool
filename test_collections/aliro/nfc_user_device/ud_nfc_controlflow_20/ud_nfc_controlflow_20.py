from aliro_actuator.access_protocol.apdu import S2, AuthenticationPolicy, Transaction
...
class UD_NFC_CONTROLFLOW_20(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "UD-NFC-CONTROLFLOW-2.0",
        "version": "0.0.1",
        "title": "UD-NFC-CONTROLFLOW-2.0",
        "description": """Verify conformance of User Device UT in CONTROLFLOW command.""",
    }
    ...
    async def setup(self) -> None:
        logger.info("UD_NFC_CONTROLFLOW_20 setup")
    ...
    async def execute(self) -> None:
        ...
        # Test step 5
        try:
            await self.reader.handle_control_flow(S2.PROTOCOL_VERSION_NOT_SUPPORTED)
        except (AccessProtocolError, InvalidResponseError) as error:
            self.mark_step_failure(str(error))
            return
        self.next_step()
    ...
    async def cleanup(self) -> None:
        logger.info("UD_NFC_CONTROLFLOW_20 Cleanup")
