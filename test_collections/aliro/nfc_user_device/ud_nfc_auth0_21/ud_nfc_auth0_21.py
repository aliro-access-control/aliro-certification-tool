...
class UD_NFC_AUTH0_21(AliroUserDeviceTestCase, UserPromptSupport):
    ...
    async def execute(self) -> None:
        ...
        first_credential_signed_timestamp = self.reader.session.credential_signed_timestamp
        first_revocation_signed_timestamp = self.reader.session.revocation_signed_timestamp
        self.next_step()
        ...
        if self.reader.session.signaling_bitmap == None or
        self.reader.session.signaling_bitmap == first_signaling_bitmap:
            self.mark_step_failure("Signaling bitmap missing or same as previous response.")
            return
        if self.reader.session.credential_signed_timestamp == None or
        self.reader.session.credential_signed_timestamp == first_credential_signed_timestamp:
            self.mark_step_failure("Credential signed timestamp missing or same as previous response.")
            return
        if self.reader.session.revocation_signed_timestamp == None or
        self.reader.session.revocation_signed_timestamp == first_revocation_signed_timestamp:
            self.mark_step_failure("Revocation signed timestamp missing or same as previous response.")
            return
        self.next_step()
