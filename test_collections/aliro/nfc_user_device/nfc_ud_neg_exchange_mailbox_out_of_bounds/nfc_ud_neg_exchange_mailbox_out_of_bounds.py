...
class ExpectedTestErrorResponse(Exception):
    """
...
class NFC_UD_NEG_EXCHANGE_MAILBOX_OUT_OF_BOUNDS(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "NFC_UD_NEG_EXCHANGE_MAILBOX_OUT_OF_BOUNDS",
        "version": "0.0.1",
        "title": "NFC_UD_NEG_EXCHANGE_MAILBOX_OUT_OF_BOUNDS",
        "description": """Expedited Phase With EXCHANGE command sending out of bounds mailbox READ request.""",
    }
    ...
    def create_test_steps(self) -> None:
        self.test_steps = [
            ...
            TestStep("Step6: Send EXCHANGE command sending out of bounds mailbox READ request and Receive EXCHANGE response"),
    ...
    async def handle_exchange_with_out_of_bounds_mailbox_read(self, 
        ...
        if response.status_code != bytes.fromhex("00020000"):
            raise ExpectedTestErrorResponse(response)
    ...
    async def execute(self) -> None:
        ...
        try:
        ...
        except ExpectedTestErrorResponse as expected_error:
            response : Response = expected_error.args[0]
            logger.info(
                "Received error status (as expected), status received: {!r}".format(
                    hexlify(response.status_code)
                )
            )
            if len(response.read_data) != 0:
                self.mark_step_failure("received unexpected data along with status_code")
                return
            pass
            ...
            self.mark_step_failure("succeed when it should have failed")
            return
    ...
    async def cleanup(self) -> None:
        logger.info("NFC_UD_NEG_EXCHANGE_MAILBOX_OUT_OF_BOUNDS Cleanup")
