...
class NFC_UD_STEPUP_AD_RD(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "NFC_UD_STEPUP_AD_RD",
        "version": "0.0.1",
        "title": "NFC_UD_STEPUP_AD_RD",
    ...
    async def execute(self) -> None:
        ...
        if not got_access:
            self.mark_step_failure("Did not receive Access Document")
            return
        if not got_revocation:
            self.mark_step_failure("Did not receive Revocation Document")
            return
    ...
    async def cleanup(self) -> None:
        logger.info("NFC_UD_STEPUP_AD_RD Cleanup")
