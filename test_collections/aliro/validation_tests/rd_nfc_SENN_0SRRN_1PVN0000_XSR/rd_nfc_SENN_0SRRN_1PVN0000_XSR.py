    ...
    async def execute(self) -> None:
        ...
        while True:
            ...
            if cmds_exchange.ins == INS.EXCHANGE:
                ...
                return
        if cmds_exchange.reader_status is None:
            self.mark_step_failure("expected reader status, but received none")
            return
        logger.info(
            "received {} reader status ".format(len(cmds_exchange.reader_status.name))
        )
