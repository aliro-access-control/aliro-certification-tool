...
class UD_NFC_SEYN_0SUNN_1PVN0000_XS0026(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "UD_NFC_SEYN_0SUNN_1PVN0000_XS0026",
        "version": "0.0.1",
        "title": "UD-NFC-SEYN.0SUNN.1PVN0000.XS0026",
    ...
    async def execute(self) -> None:
        ...
        # Test step 3
        try:
            await self.reader.transaction_initiation(check_apdu_length = True)  # including SELECT command with APDU length check
        ...
        try:
            await self.reader.handle_exchange(
                False, reader_status=ReaderStatus.HARDWARE_ISSUE
            )
        except (AccessProtocolError, InvalidResponseError) as error:
            self.mark_step_failure(str(error))
            return
    ...
    async def cleanup(self) -> None:
        logger.info("UD_NFC_SEYN_0SUNN_1PVN0000_XS0026 Cleanup")
