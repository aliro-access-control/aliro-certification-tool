...
class BLEUWB_UD_UD_DESCRIPTOR_TAG(AliroUserDeviceTestCase, UserPromptSupport):
    ...
    def create_test_steps(self) -> None:
        self.test_steps = [
            TestStep("Step1: Send Bluetooth LE advertisement"),
            TestStep("Step2: Execute AUTH0, AUTH1, EXCHANGE routines"),
            TestStep("Step3: Send Reader Status Access Protocol Completed Message ID carrying Reader Information Attribute ID"),
        ]
    ...
    async def execute(self) -> None:
        ...
        try:
            ...
            await self.reader.handle_exchange(atomic_session=False)
        except Exception as error:
            error_str = "{}: {}".format(error.__class__.__name__, repr(error))
            self.mark_step_failure(error_str)
            return
