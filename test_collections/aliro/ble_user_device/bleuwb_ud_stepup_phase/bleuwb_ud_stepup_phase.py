...
class BLEUWB_UD_STEPUP_PHASE(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "BLEUWB_UD_STEPUP_PHASE",
        "version": "0.0.1",
        "title": "BLEUWB_UD_STEPUP_PHASE",
    ...
    def create_test_steps(self) -> None:
        self.test_steps = [
            ...
            TestStep("Step6: User Device sends AP message: RSS-M2"),
            TestStep("Step7: Reader sends AP message: RSS-M3"),
            TestStep("Step8: User Device sends AP message: RSS-M4"),
            TestStep("Step9: Reader acquires UWB ranging result"),
            TestStep("Step10: Reader sends AP message: Status changed"),
        ]
    ...
    async def execute(self) -> None:
        ...
        # Test step 3: User Device sends AP message: Timesync
        try:
        ...
        # Test step 4: User Device sends AP message: Initiate Ranging
        ...
        # Test step 6: User Device sends AP message: RSS-M2
        # Test step 7: Reader sends AP message: RSS-M3
        try:
            message = await self.reader.wait_for_ble_message(
                self.reader.session.get_ble_encryption()
            )
        ...
        # Test step 8: User Device sends AP message: RSS-M4
        try:
            message = await self.reader.wait_for_ble_message(
                self.reader.session.get_ble_encryption()
            )
        ...
        # Test step 9: Reader acquires UWB ranging result
        try:
    ...
    async def cleanup(self) -> None:
        logger.info("BLEUWB_UD_STEPUP_PHASE Cleanup")
        try:
