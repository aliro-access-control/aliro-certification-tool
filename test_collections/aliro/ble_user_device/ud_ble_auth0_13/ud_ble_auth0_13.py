...
class UD_BLE_AUTH0_13(AliroUserDeviceTestCase, UserPromptSupport):
    metadata = {
        "public_id": "UD-BLE-AUTH0-1.3",
        "version": "0.0.1",
        "title": "UD-BLE-AUTH0-1.3",
    ...
    async def setup(self) -> None:
        logger.info("UD_BLE_AUTH0_13 setup")
    ...
    async def execute(self) -> None:
        ...
        second_credential_ephemeral_key = self.reader.session.credential_ephemeral_key
        if second_credential_ephemeral_key.as_bytes() == first_credential_ephemeral_key.as_bytes():
            self.mark_step_failure("Public key already used in previous command")
            return
        if self.reader.session.response_vendor_extension != None:
            self.mark_step_failure("Vendor specific extensions are present")
            return
        self.next_step()

    async def cleanup(self) -> None:
        logger.info("UD_BLE_AUTH0_13 Cleanup")
