from aliro_actuator.access_protocol.apdu import S1, S2, StatusBytes
...
class RD_NFC_CONTROLFLOW_10(AliroReaderTestCase, UserPromptSupport):
    metadata = {
        "public_id": "RD-NFC-CONTROLFLOW-1.0",
        "version": "0.0.1",
        "title": "RD-NFC-CONTROLFLOW-1.0",
    ...
    async def setup(self) -> None:
        logger.info("RD_NFC_CONTROLFLOW_10 setup")
    ...
    async def execute(self) -> None:
        ...
        try:
            ...
            return
        try:
            response = self.userdevice.apdu.create_error_response(
                StatusBytes.CONDITIONS_OF_USE_NOT_SATISFIED
            )
            await self.userdevice.transport_protocol.send_message(response)
        except AccessProtocolError as error:
            self.mark_step_failure(str(error))
            return
        self.next_step()

        # Test step 6 Receive/Send CONTROL FLOW command/response
        try:
    ...
    async def cleanup(self) -> None:
        logger.info("RD_NFC_CONTROLFLOW_10 Cleanup")
