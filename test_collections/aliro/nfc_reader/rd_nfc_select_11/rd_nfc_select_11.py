...
class RD_NFC_SELECT_11(AliroReaderTestCase, UserPromptSupport):
    metadata = {
        "public_id": "RD-NFC-SELECT-1.1",
        "version": "0.0.1",
        "title": "RD-NFC-SELECT-1.1",
        "description": """Verify conformance of Reader UT in SELECT command with wrong FCI tag.""",
    }
    ...
    async def setup(self) -> None:
        logger.info("RD_NFC_SELECT_11 setup")
    ...
    async def execute(self) -> None:
        ...
        try:
            ...
            logger.info("Creating SELECT response")
            ...
            FCI_tlv: list[tuple[int, bytes | list]] = [
                (Select.AID_TAG, cmds_select.aid),
                (Select.PROPRIETARY_TAG, proprietary.to_bytes()),
            ]

            data_tlv: list[tuple[int, bytes | list]] = [(0x6E, FCI_tlv)]
    ...
    async def cleanup(self) -> None:
        logger.info("RD_NFC_SELECT_11 Cleanup")
