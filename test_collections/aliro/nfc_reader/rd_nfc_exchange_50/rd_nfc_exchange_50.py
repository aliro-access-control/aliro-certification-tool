...
class RD_NFC_EXCHANGE_50(AliroReaderTestCase, UserPromptSupport):
    metadata = {
        "public_id": "RD-NFC-EXCHANGE-5.0",
        "version": "0.0.1",
        "title": "RD-NFC-EXCHANGE-5.0",
    ...
    async def setup(self) -> None:
        logger.info("RD_NFC_EXCHANGE_5.0 setup")
    ...
    async def execute(self) -> None:
        ...
        while True:
            ...
            if cmds_exchange.ins == INS.EXCHANGE:
                ...
                return
        if len(cmds_exchange.read_requests) == 0:
            self.mark_step_failure("expected read requests, but received none")
            return
        logger.info(
            "received {} read requests".format(len(cmds_exchange.read_requests))
        )
        for request in cmds_exchange.read_requests:
            logger.info(
                "received read requests with data: {!r}".format(hexlify(request))
            )

    async def cleanup(self) -> None:
        logger.info("RD_NFC_EXCHANGE_5.0 Cleanup")
