...
class RD_NFC_EXCHANGE_60(AliroReaderTestCase, UserPromptSupport):
    metadata = {
        "public_id": "RD-NFC-EXCHANGE-6.0",
        "version": "0.0.1",
        "title": "RD-NFC-EXCHANGE-6.0",
    ...
    async def setup(self) -> None:
        logger.info("RD_NFC_EXCHANGE_6.0 setup")
    ...
    async def execute(self) -> None:
        ...
        while True:
            ...
            if cmds_exchange.ins == INS.EXCHANGE:
                ...
                return
        if len(cmds_exchange.write_requests) == 0:
            self.mark_step_failure("expected write requests, but received none")
            return
        logger.info(
            "received {} write requests".format(len(cmds_exchange.write_requests))
        )
        for request in cmds_exchange.write_requests:
            logger.info(
                "received write requests with data: {!r}".format(hexlify(request))
            )

    async def cleanup(self) -> None:
        logger.info("RD_NFC_EXCHANGE_6.0 Cleanup")
